          ...
          {menuCategories.map((cat) => {
            ...
            return (
              <div key={cat.label} className="relative">
                <button
                  className="cursor-pointer flex items-center px-2.5 py-0.5 transition-colors"
                  style={{
                    fontFamily: "'IBM Plex Mono', monospace",
                    fontSize: "12px",
                    color: theme.menuBarText,
                    ...
                    borderRadius: "10px",
                    border: isActive
                      ? `1px solid ${theme.mode === "light" ? "#AAAAAA" : "#555"}`
                      : "1px solid transparent",
                  }}
                  onClick={() => setActiveMenu(isActive ? null : cat.label)}
                  onMouseEnter={() => {
                    if (activeMenu && activeMenu !== cat.label) setActiveMenu(cat.label);
                  }}
                >
                  {cat.label}
                </button>
                ...
                {isActive && (
                  <div
                    className="absolute left-0 top-full mt-[4px] py-1.5"
                    style={{
                      minWidth: "210px",
                      background: theme.mode === "light"
                        ? "#F5F5F5"
                        : "#2a2e34",
                      border: `1px solid ${theme.mode === "light" ? "#C4C4C4" : "#3e4248"}`,
                      borderRadius: "6px",
                  ...
                  >
                    {cat.items.map((item) => {
                      const IconComp = menuItemIcons[item.id];
                      const isHovered = hoveredItem === item.id;
                      const itemColor = isHovered
                        ? "#FFFFFF"
                        : (theme.mode === "light" ? "#222222" : "#D0D0D0");
                      const iconColor = isHovered
                        ? "#FFFFFF"
                        : (theme.mode === "light" ? "#777777" : "#888888");

                      return (
                        <button
                          key={item.id}
                          className="w-full text-left cursor-pointer flex items-center gap-2.5 transition-colors"
                          style={{
